/// Micro-op functions.
///
/// Each micro-op receives the data bus value (from the previous cycle) and
/// returns bus output pins. Named by what they DO with the data value.
///
/// fetch_opcode is always the last step. It decodes the opcode from the
/// data bus, sets tstate, increments PC, and outputs read(PC).
...
/// Consume opcode from data bus. Check interrupts. Set tstate. PC++.
/// Output: read(PC). Always the last step of every instruction.
pub fn fetch_opcode(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    if cpu.int_shift & 0x20 != 0 {
        ...
        cpu.brk_flags = 0;
        cpu.tstate = (data as u16) << 3;
        cpu.inc_pc();
    }
    read(cpu.pc)
}

/// Consume data value from data bus. Execute ALU op. PC++.
/// Output: sync_read(PC). Used as the final read step for all addressing modes.
pub fn fetch_data<OP: ReadOp>(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    OP::execute(cpu, data);
    cpu.inc_pc();
    cpu.next_state();
    sync_read(cpu.pc)
...
pub fn opcode_read(cpu: &mut Mos6502, _data: u8) -> Mos6502Output {
    cpu.next_state();
    sync_read(cpu.pc)
}
...
pub fn dummy_read(cpu: &mut Mos6502, _data: u8) -> Mos6502Output {
    cpu.next_state();
    read(cpu.pc)
}

/// Output: read(data as ZP address). No state change.
/// Used for ZP read modes where base_addr isn't needed later.
pub fn read_zp(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    cpu.next_state();
    read(data as u16)
}

/// Save data into base_addr. Output: read(base_addr).
/// Used when base_addr is needed by later steps (ZP indexed, (Indirect),Y, ZP RMW).
pub fn latch_to_base(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    cpu.base_addr = data as u16;
    cpu.next_state();
    read(cpu.base_addr)
}
...
pub fn read_base(cpu: &mut Mos6502, _data: u8) -> Mos6502Output {
    cpu.next_state();
    read(cpu.base_addr)
}

/// Combine data as high byte with base_addr low byte. Output: read(full address).
/// Does not store the result — used when base_addr isn't needed later (abs read, ind_x read).
pub fn read_base_hi(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    cpu.next_state();
    read(cpu.base_addr | (data as u16) << 8)
}

/// Combine data as high byte with base_addr low byte. Store in base_addr. Output: read(base_addr).
/// Used when base_addr is needed by later steps (JMP (ind), abs RMW, ind_x write).
pub fn latch_to_base_hi(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    cpu.base_addr |= (data as u16) << 8;
    cpu.next_state();
    read(cpu.base_addr)
}

/// Set PC from base_addr | (data << 8). Output: sync_read(PC).
/// Used for JMP abs, JMP (ind), JSR done, RTI, BRK vector.
pub fn latch_to_pc(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    cpu.pc = cpu.base_addr | ((data as u16) << 8);
    cpu.next_state();
    sync_read(cpu.pc)
}

/// Save data as low byte into base_addr. Output: read(stack).
/// Used for RTS PCL and RTI PCL.
pub fn latch_to_base_read_stack(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    cpu.base_addr = data as u16;
    cpu.next_state();
...
pub fn inc_sp_read_stack(cpu: &mut Mos6502, _data: u8) -> Mos6502Output {
...
pub fn implied<OP: ImpliedOp>(cpu: &mut Mos6502, _data: u8) -> Mos6502Output {
    OP::execute(cpu);
    cpu.next_state();
    sync_read(cpu.pc)
}
...
pub fn accumulator<OP: RmwOp>(cpu: &mut Mos6502, _data: u8) -> Mos6502Output {
...
/// Consume ZP address from data bus. Write register. PC++.
pub fn write_zp<OP: StoreOp>(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
...
pub fn index_zp_x(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    let addr = data as u16;
    cpu.base_addr = data.wrapping_add(cpu.x) as u16;
    cpu.next_state();
    read(addr)
}
...
pub fn index_zp_y(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    let addr = data as u16;
    cpu.base_addr = data.wrapping_add(cpu.y) as u16;
    cpu.next_state();
    read(addr)
}
...
pub fn add_index_x(cpu: &mut Mos6502, _data: u8) -> Mos6502Output {
...
pub fn add_index_y(cpu: &mut Mos6502, _data: u8) -> Mos6502Output {
...
pub fn write_base<OP: StoreOp>(cpu: &mut Mos6502, _data: u8) -> Mos6502Output {
    cpu.inc_pc();
    cpu.next_state();
...
pub fn fetch_addr_lo(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    cpu.base_addr = data as u16;
    cpu.inc_pc();
    cpu.next_state();
    read(cpu.pc)
...
pub fn write_abs<OP: StoreOp>(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    cpu.base_addr |= (data as u16) << 8;
    cpu.inc_pc();
    cpu.next_state();
    write(cpu.base_addr, OP::value(cpu))
...
pub fn fetch_addr_lo_x(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    cpu.base_addr = data as u16 + cpu.x as u16;
    cpu.inc_pc();
    cpu.next_state();
    read(cpu.pc)
}
...
/// Output: read(PC) for high byte.
pub fn fetch_addr_lo_y(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    cpu.base_addr = data as u16 + cpu.y as u16;
    cpu.inc_pc();
    cpu.next_state();
    read(cpu.pc)
}
...
pub fn fetch_addr_hi_indexed(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    let page_crossed = (cpu.base_addr & 0x100) != 0;
    cpu.base_addr = cpu.base_addr.wrapping_add((data as u16) << 8);
...
pub fn fetch_addr_hi_indexed_penalty(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    let msb = (data as u16) << 8;
...
pub fn fetch_ind_lo(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
...
pub fn fetch_ind_y_lo(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    let ptr_lo = cpu.base_addr;
    cpu.base_addr = data as u16 + cpu.y as u16;
    cpu.next_state();
    read((ptr_lo.wrapping_add(1)) & 0x00FF)
...
pub fn rmw_dummy_write(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    cpu.inc_pc();
    cpu.next_state();
    write(cpu.base_addr, data)
}

/// Execute RMW op on data (the original value, re-latched from dummy write).
/// Write modified result to base_addr.
pub fn rmw_execute<OP: RmwOp>(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    let result = OP::execute(cpu, data);
    cpu.next_state();
    write(cpu.base_addr, result)
}

/// JAM: CPU halts. Does not advance tstate — stays frozen.
pub fn jam(cpu: &mut Mos6502, _data: u8) -> Mos6502Output {
    read(cpu.pc)
}

/// TAS helper: set SP = A & X, then write A & X & (addr_hi + 1).
/// Used as the fixup_write step for TAS ($9B).
pub fn tas_write(cpu: &mut Mos6502, _data: u8) -> Mos6502Output {
    cpu.sp = cpu.a & cpu.x;
...
pub fn branch<const FLAG: u8, const SET: bool>(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
...
pub fn branch_take(cpu: &mut Mos6502, _data: u8) -> Mos6502Output {
...
pub fn jmp_ind_lo(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    let target_lo = data;
...
pub fn jsr_save_lo(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    cpu.base_addr = data as u16;
    cpu.inc_pc();
    cpu.next_state();
    read(0x0100 | cpu.sp as u16)
...
/// Push PCH to stack.
pub fn jsr_push_pch(cpu: &mut Mos6502, _data: u8) -> Mos6502Output {
    let addr = 0x0100 | cpu.sp as u16;
...
/// Push PCL to stack.
pub fn jsr_push_pcl(cpu: &mut Mos6502, _data: u8) -> Mos6502Output {
    let addr = 0x0100 | cpu.sp as u16;
    cpu.sp = cpu.sp.wrapping_sub(1);
    cpu.next_state();
...
/// Output: sync_read(PC).
pub fn rts_read_pch(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    cpu.pc = cpu.base_addr | ((data as u16) << 8);
    cpu.inc_pc();
    cpu.next_state();
    sync_read(cpu.pc)
...
/// Consume P from data bus. Restore P. Increment SP. Output: read(stack).
pub fn rti_read_p(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    cpu.p = (data & !(B | U)) | U;
    let addr = 0x0100 | cpu.sp as u16;
    cpu.sp = cpu.sp.wrapping_add(1);
    cpu.next_state();
...
pub fn brk_push_pch(cpu: &mut Mos6502, _data: u8) -> Mos6502Output {
...
pub fn brk_push_pcl(cpu: &mut Mos6502, _data: u8) -> Mos6502Output {
    let addr = 0x0100 | cpu.sp as u16;
    cpu.sp = cpu.sp.wrapping_sub(1);
    cpu.next_state();
    if cpu.brk_flags & BRK_RESET != 0 {
...
pub fn brk_push_p(cpu: &mut Mos6502, _data: u8) -> Mos6502Output {
    let addr = 0x0100 | cpu.sp as u16;
    let b = if cpu.brk_flags == 0 { B | U } else { U };
    let p_val = (cpu.p & !(B | U)) | b;
    cpu.sp = cpu.sp.wrapping_sub(1);
    cpu.p |= I;
    cpu.next_state();
    if cpu.brk_flags & BRK_RESET != 0 {
...
pub fn brk_vector_lo(cpu: &mut Mos6502, _data: u8) -> Mos6502Output {
...
pub fn brk_read_vector_lo(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    cpu.base_addr = data as u16;
    let vector = match cpu.brk_flags {
...
pub fn stack_push<OP: PushOp>(cpu: &mut Mos6502, _data: u8) -> Mos6502Output {
    let addr = 0x0100 | cpu.sp as u16;
    cpu.sp = cpu.sp.wrapping_sub(1);
    cpu.next_state();
    write(addr, OP::value(cpu))
...
/// Read from stack[SP].
pub fn pull_read(cpu: &mut Mos6502, _data: u8) -> Mos6502Output {
    cpu.next_state();
    read(0x0100 | cpu.sp as u16)
}

/// Consume pulled value. Load register/flags. Output: sync_read(PC).
pub fn stack_pull<OP: PullOp>(cpu: &mut Mos6502, data: u8) -> Mos6502Output {
    OP::execute(cpu, data);
    cpu.next_state();
    sync_read(cpu.pc)
}
