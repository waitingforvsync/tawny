...
- **Interrupt pipeline:** Combined `int_shift: u16` shift register — even bits = IRQ, odd bits = NMI, shifts left by 2 each phi2. `fetch_opcode` checks bit 5 (NMI) and bit 4 (IRQ). NMI is edge-detected (`nmi_prev`), with bit 1 of `int_shift` acting as a sticky pending latch. Passes Dormann interrupt test + IRQ timing tests.
